...
//    func showProgress(message: String = "Loading...") {
//        DispatchQueue.main.async { Progress.showActivity(myView: self.view, myTitle: message) }
//    }
//
...
extension Date {
    ...
    func currentTiming() -> Timing {
        ...
        return Timing(day: day, hour: hour, minutes: min, month: month, year: year)
    }
    
    var isToday: Bool {
        
        let today = Date().currentTiming()
        let date = currentTiming()
        
        return (today.day == date.day) && (today.month == date.month) && (today.year == date.year)
    }
    
    var isCurrentYear: Bool {
        return Date().currentTiming().year == currentTiming().year
    }
...
extension String {
    ...
    var localized: String {
        guard let bundlePath = UserDefaults.standard.string(forKey: UserDefaults.AppUserDefault.appLanguage), let bundle = Bundle(path: bundlePath) else {
...
extension Double {
    func toDeciml(point: Int) -> String {
